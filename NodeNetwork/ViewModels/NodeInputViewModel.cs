...
namespace NodeNetwork.ViewModels
{
    ...
    public class NodeInputViewModel : Endpoint
    {
        ...
        #endregion
        public bool MakeConnection(object output)
        {
            //Only allow drag from output to input, not input to input as same type
            if (output is NodeOutputViewModel)
            {
                var outputModel = output as NodeOutputViewModel;
                //Dont allow connections between an input and an output on the same node
                if (outputModel.Parent != this.Parent)
                {
                    if (Connections.Count < MaxConnections || MaxConnections == 1)
                    {
                        ...
                        if (MaxConnections == Connections.Count && MaxConnections == 1)
                        {
                            ...
                            var currentConnection = Connections.Items.First();
                            //dont make new connection if the same connection already exist
                            if(currentConnection.Output == output)
                            {
                                return false;
                            }
                            //Remove the connection to this input
                            this.Parent.Parent.Connections.Remove(currentConnection);
                        }
                        else if (MaxConnections > 2)
                        {
                            // Make sure connection does not exist already.
                            if (this.Parent.Parent.Connections.Items.Any(con => con.Output == this.Parent.Parent.PendingConnection.Output && con.Input == this))
                            {
                        ...
                        {
                            //Add new connection
                            this.Parent.Parent.Connections.Add(this.Parent.Parent.ConnectionFactory(this, outputModel));
                            return true;
                        }
                    }
                }
        ...
        public NodeInputViewModel()
        {
            ...
            onConnectionChanged.Subscribe(x =>
            {
                if (x !=null)
                {
                    x.Output.Parent.WhenAnyValue(vm=>vm.Name).Select(y=>y + "." + x.Output.Name).Subscribe(y =>
                    {
                        this.NameConnection = y;
                    });
                }
                else
                {
                    this.NameConnection = String.Empty;
                }
            });
