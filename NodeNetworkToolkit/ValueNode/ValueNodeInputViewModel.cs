...
namespace NodeNetwork.Toolkit.ValueNode
{
    ...
    public class ValueNodeInputViewModel<T> : NodeInputViewModel
    {
        ...
        public T GetCurrentConnectionValue()
        {
            var connection = this.Connections.Items.FirstOrDefault();
            if (connection != null)
            {
                var output = (connection.Output as ValueNodeOutputViewModel<T>);
                if (output != null)
                {
        ...
        public bool CheckSameType(object target)
        {
            if (target == null)
            {
                return false;
            }
            if(target is ValueNodeOutputViewModel<T>)
            {
                return true;
            }
            else
        ...
        public ValueNodeInputViewModel(
            ...
            ValidationAction connectedValueChangedValidationAction = ValidationAction.DontValidate
        )
        {
            ...
            //connectedValues.ObserveOn(Scheduler.Default).Subscribe(test);
        ...
        //public InputSource<T> test= new InputSource<T>();
        private IObservable<T> GenerateConnectedValuesBinding(ValidationAction connectionChangedValidationAction, ValidationAction connectedValueChangedValidationAction)
        {
            ...
            if (connectionChangedValidationAction != ValidationAction.DontValidate)
            {
                ...
                if (connectionChangedValidationAction == ValidationAction.PushDefaultValue)
                {
                    ...
                    connectionObservables = postValidation.Select(validation =>
                    {
                        ...
                        {
                            //((ValueNodeOutputViewModel<T>)Connections.Items.First().Output).Value.ObserveOn(Scheduler.Default).Subscribe(test);
                            //connectedValues.ObserveOn(Scheduler.Default).Subscribe(test);
                    ...
                    //connectionObservables.ObserveOn(Scheduler.Default).Switch().Subscribe(test);
                }
                else
                {
            ...
            //connectedValues.Subscribe(test);
            return connectedValues;
        }      
    }
    public class InputSource<T> : IObserver<T>
    {
        public T Data;
