...
namespace NOVisionDesigner.Designer.Extensions
{
    ...
    public class BaseInputViewModel<T> : NodeInputViewModel
    {
        static BaseInputViewModel()
        {
            NNViewRegistrar.AddRegistration(() => new NodeInputView(), typeof(IViewFor<BaseInputViewModel<T>>));
        }
        ...
        public bool CheckSameGenericType(object a)
        {
            if(a is ValueNodeOutputViewModel<T>)
            {
                return true;
            }
            else
            {
                //check type inherit
                var typea = a.GetType();
                if (typea.BaseType == typeof(ValueNodeOutputViewModel<T>).BaseType)
                {

                    if (typea.GetGenericArguments()[0].IsSubclassOf(typeof(T)))
                    {
                        return true;
                    }
                }
        ...
        )
        {
            ...
            ConnectionValidator = pending => new ConnectionValidationResult(
                CheckSameGenericType(pending.Output)
                 , 
                null);
