...
{
    public class TrainOCRClassifier : TrainPythonBase
    {
        public void TrainPython(string configDir, Action<TrainingArgs> TrainUpdate)
        {
            ...
            try
            {
                run_cmd("python.exe", String.Format("\"{0}\"", System.IO.Path.Combine(MainWindow.AppPath, "Designer/Python/OCR/classify/train.py")) + " " + String.Format("\"{0}\"", configDir), TrainUpdate);
            }
            catch (Exception ex)
            {
        ...
        private void run_cmd(string cmd, string args, Action<TrainingArgs> TrainUpdate)
        {
            ...
            Regex accRegex = new Regex(@"class_output_accuracy: \S+");
            double accValue = 0;
            process.OutputDataReceived += (sender, e) =>
            {
                ...
                try
                {
                    if (e.Data.Contains("class_output_accuracy: "))
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            var accstring = matchstring.Substring("class_output_accuracy: ".Length);
