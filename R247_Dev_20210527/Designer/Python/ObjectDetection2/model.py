...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        # Inputs
        input_image1 = KL.Input(
            shape=[None, None, config.IMAGE_SHAPE[2]], name="input_image")
        input_image = KL.Lambda(lambda x: x-self.config.MEAN_PIXEL)(input_image1)
        ...
        if mode == "training":
            ...
            # Model
            inputs = [input_image1, input_image_meta,
            ...
            model = KM.Model([input_image1, input_image_meta, input_anchors],
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,step,
        ...
        #input_meta
        image_, image_meta, _, _, _ =  load_image_gt(train_dataset, self.config, 0)
        #Anchors
        anchors_ = self.get_anchors(image_.shape)
        anchors_ = np.broadcast_to(anchors_, (1,) + anchors_.shape)   
        anchors_final = anchors_.reshape([anchors_.shape[1]*anchors_.shape[2],1])
        #json data
        dictionary = {
            "image_dim" : self.config.IMAGE_MAX_DIM,
            "anchor": anchors_final.tolist(),
            "input_meta": image_meta.tolist()
        }
        json_object = json.dumps(dictionary)
  
        # Writing to sample.json
        with open(  os.path.join(self.log_dir,"anchor.json"), "w") as outfile:
