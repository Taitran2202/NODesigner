...
import random
import itertools
def image_segmentation_generator(images_path,seg_path,batch_size,input_height,input_width,output_height,output_width,subsampling=1):
    ...
    if(input_height is None):
        while True:
            ...
            for _ in range( batch_size) :
                ...
                if (subsampling>1):
                    im = cv2.resize(im, (int(im.shape[1]/subsampling),int(im.shape[0]/subsampling)))
...
def prepare_train(model  , 
        train_images , seg_images,subsampling,
        input_height=None , 
        input_width=None , 
        verify_dataset=True,
    ...
    global train_gen
    train_gen = image_segmentation_generator(train_images,seg_images,  batch_size,   model.input_height ,  model.input_width ,  model.output_height ,  model.output_width, subsampling = subsampling)
def SaveModel(model,directory):
    ...
    model_save = SegmentModel(model)
    tf.saved_model.save(SegmentModel(model), directory)
    model_proto, external_tensor_storage = tf2onnx.convert.from_function(model_save.features,
                input_signature=[tf.TensorSpec(shape=(None, None,None ,3), dtype=tf.uint8)], opset=None, custom_ops=None,
                custom_op_handlers=None, custom_rewriter=None,
                inputs_as_nchw=['input'], extra_opset=None,shape_override=None,
                target=None, large_model=False, output_path=os.path.join(directory,'model.onnx'))
def CreateModel(image_path,seg_path,modelname='unet',subsampling=1,load_weights=None):
    if(modelname=='unet'):
    ...
    prepare_train(model,train_images=image_path,seg_images=seg_path,subsampling=subsampling, load_weights = load_weights)
    return model
...
def main(argv):
    ...
    print("Subsampling rate " + str(subsampling))
    ...
    if IMAGE_ORDERING == 'channels_first':
        ...
        MERGE_AXIS = -1
    model=CreateModel( imagedir,anotationdir,modelname,subsampling)
    try:
