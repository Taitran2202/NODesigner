...
class PrintingDataset(Dataset):
    ...
    def __init__(
        ...
        self.create_average_image(normal_dir)
    ...
    def create_average_image(self,dir):
        """Create average image from the dataset.

        Returns:
            np.ndarray: Average image."""
        
        files = os.listdir(dir)
        files = [files[0]]     
        images = None   
        for image_path in files:
            image = read_image(os.path.join(dir,image_path))
            image_preprocess = self.pre_process(image=image)['image'].cpu().numpy().astype('float32')
            if(images is None):
                images = image_preprocess
            else:
                images += image_preprocess
        self.image_avg = images/len(files)
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[str, Tensor]]:
        ...
        item = {"image": (pre_processed["image"]-self.image_avg)}
...
class Printing(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage in (None, "fit"):
            self.train_data = PrintingDataset(
        ...
        self.test_data = PrintingDataset(
            normal_dir=self.normal_dir,
            abnormal_dir=self.abnormal_dir,
            split="test",
            normal_test_dir=self.normal_test,
