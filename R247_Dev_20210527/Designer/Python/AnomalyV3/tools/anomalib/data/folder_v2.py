...
def loadMetaData(fileNames:list) -> list:
    metaData=[]    
    for fileName in fileNames:
        meta_file = str(fileName)+".txt"
...
def make_dataset(
    ...
    for dir_type, path in dirs.items():
        ...
        filenames += filename
        labels += label
        metadatas += loadMetaData(filename)

    samples = DataFrame({"image_path": filenames, "label": labels, "metadata": metadatas})
...
class FolderDatasetV2(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[str, Tensor]]:
        ...
        metadata = self.samples.metadata[index]
...
class FolderV2(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage in (None, "fit"):
            self.train_data = FolderDatasetV2(
        ...
        self.test_data = FolderDatasetV2(
            normal_dir=self.normal_dir,
            abnormal_dir=self.abnormal_dir,
            split="test",
            normal_test_dir=self.normal_test,
