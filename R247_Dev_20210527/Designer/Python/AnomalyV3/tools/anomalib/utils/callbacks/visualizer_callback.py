...
class VisualizerCallback(Callback):
    ...
    def __init__(self, task: str, log_images_to: Optional[List[str]] = None, inputs_are_normalized: bool = True):
        ...
        self.prediction_data = {'anomalous':[], 'normal':[],'threshold':0.5}
    ...
    def on_test_batch_end(
        ...
        for i, (filename, image, anomaly_map, pred_score, gt_label) in enumerate(
            ...
            if (gt_label):
                self.prediction_data['anomalous'].append(float(pred_score.cpu().numpy()))
            else:
                self.prediction_data['normal'].append(float(pred_score.cpu().numpy()))
            visualizer = Visualizer()
    ...
    def on_test_end(self, _trainer: pl.Trainer, pl_module: AnomalyModule) -> None:
        ...
        """
        threshold = float(pl_module.image_threshold.value.cpu().numpy())
        self.prediction_data["threshold"] = threshold       
        min,max = pl_module.min_max.compute()
        self.prediction_data["min"] = float(min.cpu().numpy())
        self.prediction_data["max"] = float(max.cpu().numpy())
        datapath = Path(_trainer.default_root_dir) / "prediction.txt"
        with open(datapath, "w") as f:
            json.dump(self.prediction_data, f) 
