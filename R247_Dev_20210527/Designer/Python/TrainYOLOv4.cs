...
{
    public class TrainYOLOv4:TrainPythonBase
    {
        private int epochint = 0;
        private int epochtotalint = 0;
      
        public async void TrainPython(string configDir,string modelName, Action<TrainingArgs> TrainUpdate)
        {
            if (IsTrainning)
                return;
            IsTrainning = true;
            try
            {
                process = NOVisionPython.CreateTrainProcess("detection", modelName, configDir);
